...
namespace aga
{
    ...
    void EditorSceneWindow::OnAddScript ()
    {
        std::function<bool(void)> AcceptFunc = [&] {
            ...
            {
                m_Editor->GetMainLoop ()->GetSceneManager ().GetActiveScene ()->AttachScript (scriptWindow->GetName (),
                                                                                              scriptWindow->GetPath ());
                UpdateScriptsBox ();
    ...
    void EditorSceneWindow::OnAccept ()
    {
        ...
        m_Editor->UpdateSceneNameLabel (std::string ("SCENE: ") + m_NameTextBox->GetText ());
