...
int
asCompareStrings (const char* str1, size_t len1, const char* str2, size_t len2)
{
    if (len1 == 0)
    {
        if (str2 == 0 || len2 == 0)
...
double
asStringScanDouble (const char* string, size_t* numScanned)
{
...
asStringScanUInt64 (const char* string, int base, size_t* numScanned, bool* overflow)
{
    ...
    if (base == 10)
    {
        ...
        {
            if (overflow &&
                ((res > UINT64_MAX / 10) || ((asUINT (*end - '0') > (UINT64_MAX - (UINT64_MAX / 10) * 10)) && res == UINT64_MAX / 10)))
                *overflow = true;
            res *= 10;
            res += *end++ - '0';
        }
    ...
    else
    {
        ...
        if (base)
        {
            ...
            {
                if (overflow &&
                    ((res > UINT64_MAX / base) || ((asUINT (nbr) > (UINT64_MAX - (UINT64_MAX / base) * base)) && res == UINT64_MAX / base)))
                    *overflow = true;
...
//
int
asStringEncodeUTF8 (unsigned int value, char* outEncodedBuffer)
{
...
//
int
asStringDecodeUTF8 (const char* encodedBuffer, unsigned int* outLength)
{
...
//
int
asStringEncodeUTF16 (unsigned int value, char* outEncodedBuffer)
{
    if (value < 0x10000)
    {
