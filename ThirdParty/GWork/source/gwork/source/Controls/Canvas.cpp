...
Canvas::Canvas (Gwk::Skin::Base* skin)
  : ParentClass (nullptr)
  , m_bAnyDelete (false)
{
...
void
Canvas::Render (Gwk::Skin::Base* /*render*/)
{
  m_bNeedsRedraw = false;
}

void
Canvas::OnBoundsChanged (Gwk::Rect oldBounds)
{
...
void
Canvas::AddDelayedDelete (Gwk::Controls::Base* control)
{
...
void
Canvas::PreDeleteCanvas (Gwk::Controls::Base* control)
{
  if (m_bAnyDelete)
  {
...
bool
Canvas::InputMouseMoved (int x, int y, int deltaX, int deltaY)
{
  if (Hidden ())
    return false;
...
bool
Canvas::InputMouseButton (int iButton, bool bDown)
{
  if (Hidden ())
    return false;
...
bool
Canvas::InputModifierKey (int iKey, bool bDown)
{
  if (Hidden ())
    return false;
...
bool
Canvas::InputCharacter (Gwk::UnicodeChar chr)
{
  if (Hidden ())
    return false;

  // Check if character is printable, i.e. don't want hidden codes, like
  // backspace.
  if ((chr != '\t') && !std::isprint (chr))
    return false;
...
bool
Canvas::InputMouseWheel (int val)
{
  if (Hidden ())
    return false;
