...
namespace ImGuiFs
{
    ...
    const int MAX_FILENAME_BYTES = FILENAME_MAX
        * 4; // Worst case: 4 bytes per char, but huge waste of memory [we SHOULD have used imguistring.h!]
...
/*
    // Never tested (I've just been told that cl.exe does not have strcasecmp: please search the web for other possible
   alternative implementations) inline static int strcasecmp( const char *s1, const char *s2 )  { return
   _stricmp(s1,s2);
...
namespace ImGuiFs
{
    ...
    // Definitions of some helper classes (String,Path,SortingHelper,Directory). Better not expose them in the header
    // file----------
    /*
    ...
    // That's because file extensions must be returned lowercase, so that e.g. ".PNG" and ".png" can be string matched
    ...
    */
    ...
    public:
        ...
        inline static void Substr (const char* text, char* rv, int start, int count = -1)
        {
            if (!text)
                count = 0;
            if (count < 0)
                count = static_cast<int> (strlen (text)) - start;
            if (count > 0)
                strncpy (rv, &text[start], static_cast<size_t> (count));
            rv[count] = '\0';
        }
        ...
        inline static int FindLastOf (const char* text, const char toFind)
        {
            if (!text)
                return -1;
            for (int i = static_cast<int> (strlen (text)) - 1; i >= 0; i--)
            {
                if (text[i] == toFind)
                    return i;
            }
        ...
        inline static void wide_to_utf8 (const wchar_t* wstr, char* rv)
        {
            ...
            // rv[size_needed]='\0';              // If the parameter after wstr is -1, the function processes the
            // entire input string, including the terminating null character. Therefore, the resulting character string
            // has a terminating null character, and the length returned by the function includes this character.
            return;
        }
        ...
        inline static void utf8_to_wide (const char* str, wchar_t* rv)
        {
            ...
            // rv[size_needed]=L'\0';            // // If the parameter after str is -1, the function processes the
            // entire input string, including the terminating null character. Therefore, the resulting character string
            // has a terminating null character, and the length returned by the function includes this character.
            return;
        }
    ...
    public:
      ...
      // fprintf(stderr,"GetAbsolutePath(\"%s\"); (len:%d)\n",path,(int) strlen(path)); // TO remove!
            static const int bufferSize
                = DIRENT_MAX_PATH + 1; // 4097 is good (PATH_MAX should be in <limits.h>, or something like that)
        ...
        static void GetDirectoryName (const char* filePath, char* rv)
        {
            ...
            if (!filePath)
                return;
            const size_t sz = strlen (filePath);
        ...
        {
            static const int dot = static_cast<int> ('.');
            const char* p1 = strrchr (filePath, dot);
            if (!p1)
                return false;
            const size_t len = strlen (p1);
            if (len != 4)
                return false;
        ...
        static void Split (const char* path, FilenameStringVector& rv, bool leaveIntermediateTrailingSlashes = true)
        {
            ...
            if (rv.size () > 0 && strlen (rv[0]) == 0)
                strcpy (static_cast<char*> (&rv[0][0]), "/\0");
#ifdef _WIN32
    ...
    public:
        inline static bool Exists(const char* path) {
            struct stat statbuf;
            return (stat(path, &statbuf) != -1 && (S_ISREG(statbuf.st_mode)));// || (acceptLinks ?
    S_ISLNK(statbuf.st_mode) : 1));
        }
    protected:
        File() {}
    };
    */
    ...
    public:
        ...
        {
            const int isort = static_cast<int> (sorting);
            if (isort >= 0 && isort < static_cast<int> (SORT_ORDER_COUNT))
    ...
    protected:
        ...
        // Please note that calling stat(...) every time inside sorters is a suicide! And that I'm doing right that!
        // (but I guess and hope that on many systems the calls get cached somewhere: otherwise it would take ages to
        // sort)
        ...
        // Please note that calculating the file extension every time inside sorters is a suicide (well, much less than
        // before...)!
        static int IMGUIFS_CDECL Typesort (const struct dirent** e1, const struct dirent** e2)
        {
            static const int dot = static_cast<int> ('.');
            const char* p1 = strrchr (static_cast<const char*> ((*e1)->d_name), dot);
            const char* p2 = strrchr (static_cast<const char*> ((*e2)->d_name), dot);
            if (!p1)
                return (!p2 ? 0 : -1);
            else if (!p2)
                return 1;
    ...
    const SortingHelper::SorterSignature SortingHelper::Sorters[]
        = {&SortingHelper::Alphasort, &SortingHelper::Alphasortinverse, &SortingHelper::Lastmodsort,
            &SortingHelper::Lastmodsortinverse, &SortingHelper::Sizesort, &SortingHelper::Sizesortinverse,
            &SortingHelper::Typesort, &SortingHelper::Typesortinverse};
    ...
    public:
        static void GetDirectories (const char* directoryName, PathStringVector& result,
            FilenameStringVector* pOptionalNamesOut = nullptr, Sorting sorting = SORT_ORDER_ALPHABETIC)
        {
            result.clear ();
        ...
        static void GetFiles (const char* directoryName, PathStringVector& result,
            FilenameStringVector* pOptionalNamesOut = nullptr, Sorting sorting = SORT_ORDER_ALPHABETIC)
        {
            result.clear ();
            if (pOptionalNamesOut)
        ...
        static void GetFiles (const char* path, PathStringVector& files, const char* wantedExtensions,
            const char* unwantedExtensions = nullptr, FilenameStringVector* pOptionalNamesOut = nullptr,
            Sorting sorting = SORT_ORDER_ALPHABETIC)
        {
            ...
            if ((wantedExtensions == nullptr || strlen (wantedExtensions) == 0)
                && (unwantedExtensions == nullptr || strlen (unwantedExtensions) == 0))
            {
                files = filesIn;
                return;
            }
        ...
        inline static const PathStringVector& GetUserKnownDirectories (
            ...
            const int** pOptionalNumberKnownUserDirectoriesExceptDrives = nullptr, bool forceUpdate = false)
        {
            ...
            dn.clear ();
#ifdef _WIN32
            static const int csid[] = {CSIDL_DESKTOP,
                CSIDL_PERSONAL, //(Documents)
                CSIDL_FAVORITES, CSIDL_MYMUSIC, CSIDL_MYPICTURES, CSIDL_RECENT, CSIDL_MYVIDEO};
            static const char* name[] = {"Desktop", "Documents", "Favorites", "Music", "Pictures", "Recent", "Video"};
            ...
            numberKnownUserDirectoriesExceptDrives = static_cast<int> (rv.size ());
            // Additional Drives --------------------------------------------
            ...
            static const bool ifHomeSubfolerIsFoundInMountLocationsForgetThatRootMountLocation
                = true; // That means: if "/media/myusername" exists, don't add any other "/media/..." entries.
    ...
    struct UnZipFileImpl
    {
        ...
        struct unz_file_info64_plus
        {
            ...
            static void Sort (ImVector<unz_file_info64_plus>& fileInfos, Sorting sorting)
            {
                ...
                switch (sorting)
                {
                case SORT_ORDER_ALPHABETIC:
                    sorter = &unz_file_info64_plus::Alphasort;
                    break;
                case SORT_ORDER_ALPHABETIC_INVERSE:
                    sorter = &unz_file_info64_plus::Alphasortinverse;
                    break;
                case SORT_ORDER_LAST_MODIFICATION:
                    sorter = &unz_file_info64_plus::LastModsort;
                    break;
                case SORT_ORDER_LAST_MODIFICATION_INVERSE:
                    sorter = &unz_file_info64_plus::LastModsortinverse;
                    break;
                case SORT_ORDER_SIZE:
                    sorter = &unz_file_info64_plus::Sizesort;
                    break;
                case SORT_ORDER_SIZE_INVERSE:
                    sorter = &unz_file_info64_plus::Sizesortinverse;
                    break;
                case SORT_ORDER_TYPE:
                    sorter = &unz_file_info64_plus::Typesort;
                    break;
                case SORT_ORDER_TYPE_INVERSE:
                    sorter = &unz_file_info64_plus::Typesortinverse;
                    break;
                default:
                    break;
                }
            ...
            {
                return strcasecmp (static_cast<const unz_file_info64_plus*> (e1)->name,
        ...
        bool getFilesOrDirectories (bool fileMode, const char* directoryName, PathStringVector& result,
            ...
            bool prefixResultWithTheFullPathOfTheZipFile) const
        {
            ...
            while (dirNameLen > 0 && (dirName[dirNameLen - 1] == '/' || dirName[dirNameLen - 1] == '\\'))
            {
            ...
            for (i = 0; i < gi.number_entry; i++)
            {
                ...
                const bool isDirectory = filename_inzip_len > 0
                ...
                if (ok)
                {
                    ...
                    if (ok)
                    {
                        ...
                        strcpy (tmp,
                            dirNameLen > 0
                                ? &filename_inzip[dirNameLen + 1]
                                : filename_inzip); // Here "tmp" if the plain name, and "filename_inzip" the full name
            ...
            /*
             if ((file_info.flag & 1) != 0) strcat(tmp,charCrypt);
                                ...
                                if (pOptionalNamesOut)  {
                                    ...
                                    char* pName = &(*pOptionalNamesOut)[resultSize][0];
                                    strcpy(pName,tmp);
                                    if ((file_info.flag & 1) != 0) strcat(pName,charCrypt); // display a '*' if the file
             is crypted
        ...
        static bool PathExistsWithZipSupport (const char* path, bool reportOnlyFiles, bool reportOnlyDirectories,
            bool checkAbsolutePath = true, bool* isInsideAZipFile = NULL)
        {
    ...
    bool UnZipFile::getDirectories (const char* directoryName, PathStringVector& result,
        FilenameStringVector* pOptionalNamesOut, Sorting sorting, bool prefixResultWithTheFullPathOfTheZipFile) const
    {
    ...
    bool UnZipFile::getFiles (const char* directoryName, PathStringVector& result,
        FilenameStringVector* pOptionalNamesOut, Sorting sorting, bool prefixResultWithTheFullPathOfTheZipFile) const
    {
        return im->getFilesOrDirectories (
    ...
    unsigned int UnZipFile::getFileSize (const char* filePath) const
    {
        ...
        int err
            = unzGetCurrentFileInfo64 (im->uf, &file_info, filename_inzip, sizeof (filename_inzip), NULL, 0, NULL, 0);
        if (err != UNZ_OK)
            return 0;
    ...
    static bool UnZipFileGetFileContentBase (
        ImGuiFs::UnZipFileImpl* im, const char* filePath, ImVector<CharType>& bufferOut, const char* password)
    {
        bufferOut.clear ();
        if (!im->uf || !filePath)
            return false;
        ...
        {
            fprintf (
                stderr, "Error while unzipping: \"%s\": %d with zipfile in unzGetCurrentFileInfo\n", filePath, err);
            return false;
        }
    ...
    bool UnZipFile::getFileContent (const char* filePath, ImVector<char>& bufferOut, const char* password) const
    {
    ...
    bool UnZipFile::fileExists (const char* pathInsideZip) const { return exists (pathInsideZip, true, false); }
    bool UnZipFile::directoryExists (const char* pathInsideZip) const { return exists (pathInsideZip, false, true); }
    ...
    bool PathExistsWithZipSupport (const char* path, bool reportOnlyFiles, bool reportOnlyDirectories,
        bool checkAbsolutePath, bool* isInsideAZipFile)
    {
    ...
    bool DirectoryGetDirectoriesWithZipSupport (const char* directoryName, PathStringVector& result,
        FilenameStringVector* pOptionalNamesOut, Sorting sorting, bool prefixResultWithTheFullPathOfTheZipFile)
    {
        result.clear ();
        if (pOptionalNamesOut)
            pOptionalNamesOut->clear ();
    ...
    bool DirectoryGetFilesWithZipSupport (const char* directoryName, PathStringVector& result,
        FilenameStringVector* pOptionalNamesOut, Sorting sorting, bool prefixResultWithTheFullPathOfTheZipFile)
    {
        result.clear ();
        if (pOptionalNamesOut)
    ...
    bool FileGetContentBase (const char* path, ImVector<CharType>& bufferOut, bool openInTextMode, const char* password)
    {
        ...
        const size_t sz = static_cast<size_t> (szl);
        if ((sizeof (long) > sizeof (size_t) && szl != static_cast<long> (sz))
            || (sizeof (long) < sizeof (size_t) && static_cast<size_t> (szl) != sz))
        {
        ...
        if (sz > 0)
        {
            if (bufferOut.size () >= static_cast<int> (sz))
                fread (&bufferOut[0], static_cast<size_t> (sz), 1, fin);
            else
            {
                fprintf (stderr, "Error in: FileGetContent(\"%s\"): file too big.\n", mainPath);
                bufferOut.clear ();
    ...
    struct ImGuiFsDrawIconStruct
    {
        ...
        inline bool drawIcon (int extensionType, const ImVec4* pOptionalColorOverride = nullptr) const
        {
        ...
        inline bool drawIcon (
            const char* ext, bool caseSensitiveMatch = false, const ImVec4* pOptionalColorOverride = nullptr) const
        {
    ...
    struct FolderInfo
    {
        ...
        void fromCurrentFolder (const char* path)
        {
            ...
            else
            {
                ...
                splitPathIndex = static_cast<int> (SplitPath.size ()) - 1;
        ...
        int getSplitPathIndexFor (const char* path /*,int* pOptionalSplitPathIndexOfZipFile=NULL*/) const
        {
            ...
            char tmp[MAX_PATH_BYTES];
            tmp[0] = '\0';
            for (int i = 0, sz = static_cast<int> (SplitPath.size ()); i < sz; i++)
            {
        ...
        {
            Path::Split (fullFolder, SplitPath);
            const int splitPathSize = static_cast<int> (SplitPath.size ());
    ...
    public:
        ...
        {
            return currentInfoIndex >= 0 && currentInfoIndex < static_cast<int> (info.size ()) - 1;
        }
        void reset ()
        {
            info.clear ();
        ...
        {
            return (currentInfoIndex >= 0 && currentInfoIndex < static_cast<int> (info.size ()));
        }
        const FolderInfo* getCurrentFolderInfo () const { return isValid () ? &info[currentInfoIndex] : nullptr; }
        const char* getCurrentFolder () const
        {
            return isValid () ? &info[currentInfoIndex].currentFolder[0] : nullptr;
        }
        ...
        {
            return isValid () ? &info[currentInfoIndex].splitPathIndex : nullptr;
        }
        size_t getInfoSize () const { return static_cast<size_t> (info.size ()); }
    };
       ...
       //-----------------------------------------------------
        ...
        inline void calculateBrowsingDataTableSizes (const ImVec2& childWindowSize = ImVec2 (-1, -1))
        {
            ...
            if (childWindowSize.y > 0)
            {
                ...
                // static int tmp = 0;if (tmp!=numLinesThatFit) {tmp=numLinesThatFit;fprintf(stderr,"childWindowSize.y =
    ...
    Dialog::DrawFileIconDelegate Dialog::DrawFileIconCallback = nullptr;
    Dialog::DrawFolderIconDelegate Dialog::DrawFolderIconCallback = nullptr;

    Dialog::Dialog (bool noKnownDirectoriesSection, bool noCreateDirectorySection, bool noFilteringSection,
        bool detectKnownDirectoriesAtEachOpening, bool addDisplayByOption, bool dontFilterSaveFilePathsEnteredByTheUser)
    {
        internal = static_cast<Internal*> (ImGui::MemAlloc (sizeof (Internal)));
    ...
    const char* ChooseFileMainMethod (Dialog& ist, const char* directory, const bool _isFolderChooserDialog,
        const bool _isSaveFileDialog, const char* _saveFileName, const char* fileFilterExtensionString,
        const char* windowTitle, const ImVec2& windowSize, const ImVec2& windowPos, const float windowAlpha)
    {
        //-----------------------------------------------------------------------------
        ...
        //----------------------------------------------------------
        static const int* pNumberKnownUserDirectoriesExceptDrives = nullptr;
        static const FilenameStringVector* pUserKnownDirectoryDisplayNames = nullptr;
        ...
        // Fill ColorSet above and fix dummyButtonColor here
        {
            ...
            for (int i = 0, sz = static_cast<int> (Internal::ImGuiCol_Dialog_Directory_Text); i <= sz; i++)
            {
                ImVec4& c = ColorSet[i];
                const ImVec4& r = style.Colors[i < sz ? (static_cast<int> (ImGuiCol_Button) + i) : ImGuiCol_Text];
                Internal::ColorCombine (c, r, df);
                if (i < sz)
                    c.w *= df.w;
            }
            for (int i = static_cast<int> (Internal::ImGuiCol_Dialog_File_Background),
                     sz = static_cast<int> (Internal::ImGuiCol_Dialog_File_Text);
                 i <= sz; i++)
            {
                ImVec4& c = ColorSet[i];
                const ImVec4& r
                    = style.Colors[i < sz ? (static_cast<int> (ImGuiCol_Button)
                                                - static_cast<int> (Internal::ImGuiCol_Dialog_File_Background) + i)
                                          : ImGuiCol_Text];
                Internal::ColorCombine (c, r, ff);
                if (i < sz)
                    c.w *= ff.w;
            }
            for (int i = static_cast<int> (Internal::ImGuiCol_Dialog_ZipDirectory_Background),
                     sz = static_cast<int> (Internal::ImGuiCol_Dialog_ZipDirectory_Text);
                 i <= sz; i++)
            {
                ImVec4& c = ColorSet[i];
                const ImVec4& r = style.Colors[i < sz ? (static_cast<int> (ImGuiCol_Button) + i) : ImGuiCol_Text];
        ...
        if (I.rescan && !I.openedOnce)
        {
            ...
            I.wndPos.x = screenSize.x * 0.5f - I.wndSize.x * 0.5f;
            I.wndPos.y = screenSize.y * 0.5f - I.wndSize.y * 0.5f;
        ...
        if (I.forceRescan)
        {
            ...
            const int sortingModeForDirectories
                = (I.sortingMode <= static_cast<int> (SORT_ORDER_LAST_MODIFICATION_INVERSE)) ? I.sortingMode
                                                                                             : (I.sortingMode % 2);
            Directory::GetDirectories (I.currentFolder, I.dirs, &I.dirNames,
                static_cast<Sorting> (
                    sortingModeForDirectories)); // this is because directories don't return their size or their
                                                 // file extensions (so if needed we sort them alphabetically)

            if (!isSelectFolderDialog)
            {
                if (!fileFilterExtensionString || strlen (fileFilterExtensionString) == 0)
                    Directory::GetFiles (I.currentFolder, I.files, &I.fileNames, static_cast<Sorting> (I.sortingMode));
                else
                    Directory::GetFiles (I.currentFolder, I.files, fileFilterExtensionString, nullptr, &I.fileNames,
                        static_cast<Sorting> (I.sortingMode));
...
#else // IMGUI_USE_MINIZIP
            const int sortingModeForDirectories
                = (I.sortingMode <= (int)SORT_ORDER_LAST_MODIFICATION_INVERSE) ? I.sortingMode : (I.sortingMode % 2);
            char basePath[MAX_PATH_BYTES];
            basePath[0] = '\0';
            char zipPath[MAX_PATH_BYTES];
            zipPath[0] = '\0';
            ...
            if (!isInsideZipFile)
                Directory::GetDirectories (basePath, I.dirs, &I.dirNames,
                    (Sorting)sortingModeForDirectories); // this is because directories don't return their size or their
                                                         // file extensions (so if needed we sort them alphabetically)
            else
            {
                if (I.unz.load (basePath, false))
                    I.unz.getDirectories (zipPath, I.dirs, &I.dirNames, (Sorting)sortingModeForDirectories, true);
                else
                    I.unz.close ();
            }
            ...
            if (!isSelectFolderDialog)
            {
                ...
                {
                    /*if (!fileFilterExtensionString || strlen(fileFilterExtensionString)==0)*/ I.unz.getFiles (
                        zipPath, I.files, &I.fileNames, (Sorting)I.sortingMode, true);
                    // else
            ...
            I.totalNumBrowsingEntries = static_cast<int> (I.dirs.size () + I.files.size ());
            ...
            if (I.history.getInfoSize () > 0)
                fprintf (stderr,
                    ...
                    I.currentFolder, I.history.canGoBack () ? "true" : "false",
...
#undef DEBUG_HISTOTY
        ...
        const bool isBrowsingInsideZipFile
            = (fi && (!(fi->splitPathIndexOfZipFile < 0 || fi->splitPathIndexOfZipFile > fi->splitPathIndex)));
        ...
        bool editLocationInputTextReturnPressed = false;
        {
            ...
            if (ImGui::Button ("L##EditLocationCheckButton"))
            {
                ...
                // if (!I.editLocationCheckButtonPressed) mustValidateInputPath = true;   // or not ? I mean: the user
                // wants to quit or to validate in this case ?
            }
            ...
            if (I.editLocationCheckButtonPressed)
            {
                ...
                editLocationInputTextReturnPressed
                    = ImGui::InputText ("##EditLocationInputText", I.editLocationInputText, MAX_PATH_BYTES,
       ...
       // else fprintf(stderr,"mustValidateInputPath NOOP: \"%s\" \"%s\"\n",I.currentFolder,cleanEnteredPath);
        ...
        if (!I.editLocationCheckButtonPressed && !editLocationInputTextReturnPressed)
        {
            ...
            // Tab:
            {
                ...
                //-----------------------------------------------------
                {
                    const int numTabs = static_cast<int> (I.currentSplitPath.size ());
        ...
        if (I.allowKnownDirectoriesSection && pUserKnownDirectories->size () > 0)
        {
            ...
            if (ImGui::CollapsingHeader ("Known Directories##imguifs_UserKnownDirectories"))
            {
                ...
                for (int i = 0, sz = static_cast<int> (pUserKnownDirectories->size ()); i < sz; i++)
                {
                    ...
                    // if (Dialog::DrawFolderIconCallback && Dialog::DrawFolderIconCallback(false,&iconsColor))
                    // ImGui::SameLine();
...
#undef SIMULATING_ONLY
        ...
        ImGui::SameLine ();
        {
            ...
            //-----------------------------------------------------
            {
                static const int numTabs = static_cast<int> (SORT_ORDER_COUNT) / 2;
        ...
        //-----------------------------------------------------------------------------
        {
                            ...
                            : 0)))
            {
                ...
                I.calculateBrowsingDataTableSizes (
                    ImGui::GetWindowSize ()); // (Actually we could save this call every frame, but it's difficult to
                                              // detect whn we need it)

                // ImGui::SetScrollPosHere();   // possible future ref: while drawing to place the scroll bar making a
                // certain entry visible Also we don't do any maual clipping here (for long directories using a clipper
                // might be useful)
                ...
                if (I.dirs.size () > 0)
                {
                    ...
                    for (int i = 0, sz = static_cast<int> (I.dirs.size ()); i < sz; i++)
                    {
                ...
                if (!isSelectFolderDialog && I.files.size () > 0)
                {
                    ...
                    for (int i = 0, sz = static_cast<int> (I.files.size ()); i < sz; i++)
                    {
        ...
        if (isSaveFileDialog || isSelectFolderDialog)
        {
            ...
            if (isSaveFileDialog)
            {
                ...
                lastTwoButtonsWidth = ImGui::CalcTextSize ("Save Cancel").x
                    + 2.0f * (style.FramePadding.x + style.ItemSpacing.x) + style.WindowPadding.x;
            ...
            else
            {
                ...
                static const ImVec4 sf (1.0, 0.8f, 0.5, 1); // selected folder color factor
                ...
                ImGui::PushItemWidth (ImGui::GetContentRegionAvailWidth () - lastTwoButtonsWidth);
                ImGui::InputText (
                    "##selectFileNameDialog", &I.saveFileName[0], MAX_FILENAME_BYTES, ImGuiInputTextFlags_ReadOnly);
                ImGui::PopItemWidth ();
                ImGui::SameLine ();
            }
    ...
    const char* Dialog::chooseFileDialog (bool dialogTriggerButton, const char* directory,
    ...
    const char* Dialog::saveFileDialog (bool dialogTriggerButton, const char* directory,
        const char* startingFileNameEntry, const char* fileFilterExtensionString, const char* windowTitle,
        const ImVec2& windowSize, const ImVec2& windowPos, const float windowAlpha)
    {
        if (dialogTriggerButton)
        {
